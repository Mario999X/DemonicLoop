...
public class Enemy_Mechanics : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (_Battle == null)
        {
            _Battle = GameObject.Find("System").GetComponent<Enter_Battle>();
        }
        ...
        if (Physics.Raycast(transform.position, direction, out hit, distance, layer))
        {
            ...
            hit.transform.GetComponent<Player_Move>().enabled = false; // Desactiva el movimiento del jugador.

            // Si encuentra alguno de los dos componentes en el objeto jugador los desactiva antes de iniciar la pelea.
            if (hit.transform.GetComponent<KeyBoard_Controls>())
                hit.transform.GetComponent<KeyBoard_Controls>().enabled = false;
            if (hit.transform.GetComponent<Controller_Controls>())
                hit.transform.GetComponent<Controller_Controls>().enabled = false;
